 ...
 *
 * Chạy: node src/scripts/seed-statistics-data.js
 *
 * LƯU Ý:
...
async function seedStatisticsData() {
  try {
    ...
    if (!user) {
      throw new Error('Không tìm thấy user để gán dữ liệu thống kê. Hãy chạy seed.js trước.');
    }

    console.log('👤 Sử dụng user cho thống kê:', user.email);
